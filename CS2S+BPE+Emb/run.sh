...
CUDA_VISIBLE_DEVICES=$device python $FAIRSEQPY/generate.py --no-progress-bar --path $models --beam $beam --nbest $beam --interactive --workers $threads $TRAINING_DIR/processed/bin/ < $output_dir/input.bpe.txt > $output_dir/output.bpe.nbest.txt
